...
def create_gScore(self):
    ...
    # be set to infinity.
    gScoreDict = {}
    for keys in self.map.intersections:
        gScoreDict[keys] = float('inf')
    
    gScoreDict[self.start] = 0.0
    
    #print("GSCORES : ",gScore , "---gSCORE END--")
...
def create_fScore(self):
    ...
    fScoreDict = {}
    for keys in self.map.intersections:
        fScoreDict[keys] = float('inf')
        
    fScoreDict[self.start] = self.heuristic_cost_estimate(self.start)
    #print(fScoreDict)
    return fScoreDict
...
def get_current_node(self):
    ...
    #return min(self.openSet)
    currentNode = None
    currentMin = float('inf')
    for i in self.openSet:
        if(currentNode==None):
            currentNode = i
            currentMin = self.calculate_fscore(currentNode)
        elif (self.calculate_fscore(currentNode) < currentMin):
            currentMin = self.calculate_fscore(currentNode)
            currentNode = i
    return currentNode
...
def distance(self, node_1, node_2):
    ...
    #return 2.0*(math.sqrt((self.map.intersections[node_1]**2)+(self.map.intersections[node_2]**2)))
